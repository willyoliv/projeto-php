...
<body>
  ...
  <div class="container">
    ...
    <form class="form" method="post" action="{{route('disciplinas.destroy',$disciplina->id)}}">
      {!! method_field('DELETE') !!}
      {!! csrf_field() !!}
      <p class="form-control">{{$disciplina->name}}</p>
      <p class="form-control">{{$disciplina->criacao}}</p>
      <button type="submit" class="btn btn-danger btn-lg btn-block btn-outline-">Deletar Disciplina</button><br>
    </form>
  </div>
</body>
</html>
